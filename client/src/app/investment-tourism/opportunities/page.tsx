...
export default function OpportunitiesPage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 font-sans flex flex-col justify-between">
      <div>
        <Header />
        <section className="bg-gradient-to-br from-green-900 via-green-800 to-green-950 text-white py-8 sm:py-10 text-center overflow-x-hidden">
          <div className="container mx-auto px-4 sm:px-6">
            <h1 className="text-3xl md:text-4xl font-black">{t.investmentTourism.opportunities}</h1>
            <p className="mt-2 text-emerald-100/80 max-w-xl mx-auto text-sm">{t.investmentTourism.opportunitiesDesc}</p>
          </div>
        </section>
        <main className="container mx-auto px-4 sm:px-6 py-6 sm:py-8 max-w-4xl">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-8">
            <ul className="space-y-4">
              {items.map((item, i) => (
                <li key={i} className="flex items-start gap-4 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                  <span className="shrink-0 w-8 h-8 rounded-lg bg-slate-700 dark:bg-slate-600 text-white flex items-center justify-center text-sm font-bold">{i + 1}</span>
