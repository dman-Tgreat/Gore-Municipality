...
function NavDropdown({ items, label, href, isActive, isOpen, onMouseEnter, onMouseLeave, pathname, t, dynamicItems, dynamicLabel }: NavDropdownProps) {
  ...
  return (
    <div className="relative" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      {/* Label is a Link to the main page — clicking navigates directly */}
      <Link
        href={href}
      ...
      {isOpen && (
        <div className="absolute left-0 mt-2 w-64 bg-white rounded-xl shadow-xl border border-gray-100 py-2 overflow-hidden z-50">
          {/* Static items */}
          {items.map((item) => (
            <Link
              key={item.href}
